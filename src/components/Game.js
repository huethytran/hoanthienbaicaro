...
class Game extends React.Component{
    constructor(){
      ...
      handleClick(i, winner){
        ...
        this.state.history.splice(this.state.numOfStep , Math.abs(this.state.backStep));
      ...
      handleHistoryClick(i)
      {
        if ( i < this.state.history.length -1 + this.state.backStep)
        {
          ...
          for (var j = this.state.history.length -1; j>i; j--)
            this.state.squares[this.state.history[j].step] = null;
        }
        if ( i > this.state.history.length -1 + this.state.backStep)
        {
          for (var j = this.state.history.length + this.state.backStep; j <= i; j++)
            this.state.squares[this.state.history[j].step] = this.state.history[j].player;
          this.state.backStep = - this.state.history.length + i +1;
        }
    ...
    let status;
    if(winner.kq) status = "Winner is: " + winner.kq; // Nếu winner có giá trị thì sẽ hiển thị người thắng cuộc
     else
      ...
      return(
        ...
        <Button type="dashed" size="large" shape="round" onClick={this.replay}>Play Again</Button>
          ...
          <div className="game"><Board preStep = {this.state.preStep} squares={this.state.squares} history={this.state.history} backStep={this.state.backStep} winner={winner} onClick={i => this.handleClick(i, winner.kq)} xIsNext={this.state.xIsNext} /></div> 
          <div className="divhistory">
            <div className="divhistory1">
            <p>&nbsp;&nbsp;History</p>
            <Button className="btnSort" type="primary" size="large" onClick={this.handleSortButtonClick}>Sort</Button>
            </div>
            <div className="history">
             <History history={this.state.history} backStep={this.state.backStep} isAsc={this.state.isAsc} onClick={i => this.handleHistoryClick(i)}></History>
            </div>
          </div>
        </div>
      );
