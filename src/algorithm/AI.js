...
function nextBestMoveEasyMode(board, boardSize) {
  let m;
  let l;
  let position;
  let type;
    ...
    for (var j = 4; j--; ) {
      ...
      for (var k = !board[i] ? 5 : 1; k--; ) {
        ...
        if (line.length === 7 && undefined !== type) {
          ...
          if (score > directionScore[0]) {
            directionScore[0] = score;
          }
        }
      }
...
//  Size = Integer
//  Position = { x: Integer, y: Integer }
export function bestMove(board, boardSize, type) {
  let pos;
  if (type === 1) {
    console.log('easyyy');
    pos = nextBestMoveEasyMode(board, boardSize); // easy mode
  } else pos = nextBestMove(board, boardSize);
  console.log('aloooo', pos);
